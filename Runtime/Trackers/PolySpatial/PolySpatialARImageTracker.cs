...
namespace Unity.PolySpatial.XR.Internals
{
    class PolySpatialARImageTracker : IDisposable
    {
        ...
        void OnChanged(ARTrackedImagesChangedEventArgs eventArgs)
        {
            ...
            {
                var output = CreatePolySpatialARTrackedImage(image, ARTrackedImageOperation.Removed);
                if (output != null)
        ...
        PolySpatialARTrackedImage CreatePolySpatialARTrackedImage(ARTrackedImage image, ARTrackedImageOperation operation)
        {
            ...
            var bytes = textureGuid.ToByteArray();
            var guidLow = BitConverter.ToUInt64(bytes, 0);
            var guidHigh = BitConverter.ToUInt64(bytes, 8);

            var serializableTextureGuid = new SerializableGuid(guidLow, guidHigh);
